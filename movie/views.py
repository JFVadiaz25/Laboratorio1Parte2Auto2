...
def home(request):
    ...
    #return render(request, 'home.html',{'name':'Juan Felipe Vanegas Diaz '})
...
def statistics_view(request): 
    ...
    # Obtener todas las películas
    all_movies = Movie.objects.all()
    # Crear un diccionario para almacenar la cantidad de películas por género (solo el primer género)
    ...
    buffer_genre.close()
    graphicgenre = base64.b64encode(image_png_genre)
    graphicgenre = graphicgenre.decode('utf-8')

    # Renderizar la plantilla statistics.html con ambas gráficas
    return render(request, 'statistics.html', {'graphic': graphic, 'graphicGenre': graphicgenre})
